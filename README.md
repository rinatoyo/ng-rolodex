...
1. Generate a project using: `ng new ng-rolodex`
1. When prompted, "Would you like to add Angular routing?", pick yes
1. When prompted, "Which Stylesheet format would you like to use?", pick SCSS
1. Change into dir: `cd ng-rolodex`
1. Install all appropriate docker-based files
1. Install the `knex`, `bookshelf`, and `pg` dependencies using npm
1. Setup `knexfile.js` with `knex init`
1. Setup the appropriate `knex.js` and `bookshelf.js` files
1. Create 2 migrations, `Users` and `Contacts` _refer to **[schemas](#schemas)**_
1. Create the `User` and `Contact` Bookshelf model files
1. Install the `express` and `body-parser` dependency using npm
1. Setup an express server in `server.js`
1. Set up express middleware for `body-parser`
...
The user can logout. On logout, delete the `user_id` value from `localStorage` and redirect the user back to the login route `/login`.
...
The `/contacts` route will list all contacts for the logged in user.

The all contacts should links to the appropriate `/contact/:id` route.
